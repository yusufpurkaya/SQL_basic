...
select count(ogrno) as ogrencisayisi from ogrenci;
...
select kitap_turu,count(*) sayi from islemler group by kitap_turu;
-------- Örnek 5: Her türdeki en fazla sayfa sayısı olan kitapları listeleyiniz.
select kitap_turu,max(sayfasayisi) from kitap k,islemler i where i.turno=k.turno group by i.turno;
-------- Örnek 6: Her türdeki en az sayfa sayısı olan kitapları listeleyiniz.
select kitap_turu,min(sayfasayisi) from kitap k,islemler i where i.turno=k.turno group by i.turno;
...
-------- Örnek 7***: Her türden kaç tane kitap olduğunu listeleyiniz.(Tür isimleri de olsun)
 select kitap_turu,count(k.turno) from islemler i,kitap k where i.turno=k.turno group by k.turno order by i.kitap_turu;